...
__KT GiGA WiFi 사용자 설정 페이지의 (시스템?)로그(부팅 후 많은 시간이 지나면 다른 로그에 의해 확인할 수 없는 경우가 있음)를 확인한 후, 'Update' 또는 'Upgrade' 등의 문자열을 포함하는 로그가 아래의 4가지 규칙 추가 & 재부팅 후 2-5분 후에 기록되는 로그에 차이점이 있는지 확인하고 알려주셨으면 합니다.__
