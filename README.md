...
KT GiGA WiFi (통신사 공유기, 기본값 설정인 KT 모드가 필요) - 제한에서 벗어날 각 기기(이중 NAT 구성의 사제 공유기 포함)에 대해 수동IP설정* 또는 GiGA WiFi의 사용자 설정 페이지에서 '수동 IP 할당 설정'(DHCP 정적 할당) 설정을 완료

이중 NAT 구성의 사제 공유기(KT GiGA WiFi의 컴퓨터/LAN 포트와 사제 공유기의 WAN/인터넷 포트를 연결한 경우)는 해당 공유기의 수동IP설정\* 또는 '수동 IP 할당 설정'을 통해 사제 공유기와 연결한 기기의 인터넷 연결이 영향을 받습니다.
...
__KT GiGA WiFi 사용자 설정 페이지 - 상태정보의 로그(부팅 후 많은 시간이 지나면 다른 로그에 의해 확인할 수 없는 경우가 있음)를 확인한 후, 'Update' 또는 'Upgrade' 등의 문자열을 포함하는 로그가 아래의 4가지 규칙 추가 & 재부팅 후 2-5분 후에 기록되는 로그에 차이점이 있는지 확인하고 알려주셨으면 합니다.__
